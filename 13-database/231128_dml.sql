...
insert into customer (addr, custid, custname, addr, phone, birth)
...
-- 여러 튜플 동시에 추가
insert into customer
values ('banana', '김유정', '미국 뉴욕', '01022223333', '2002-10-05'),
('orange', '송강', '미국 뉴욕', '01022223333', '2002-10-05'),
('kiwi', '박은빈', '미국 뉴욕', '01022223333', '2002-10-05'),
('grape', '윤지성', '미국 뉴욕', '01022223333', '2002-10-05');
...
-- 고객 이름의 두번째 글자가 '지'인 고객 검색
-- _ : 임의의 한글자(하나의) 문자를 의미
select * from customer where custname like '_지%';
select * from customer where custname like '_지'; -- 이름이 두글자인 x지 라는 고객

-- 고객 이름의 세번째 글자가 '수'인 고객 검색
select * from customer where custname like '__수%';
select * from customer where custname like '%수'; -- 이름이 몇자든 마지막 글자가 '수'인 고객

-- 복합 조건(AND, OR, NOT)
-- 주소지가 대한민국이고, 2000년대 이후 출생 고객 검색
