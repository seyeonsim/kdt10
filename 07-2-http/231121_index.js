...
const server = http.createServer(function (req, res) {
    ...
    // 예외처리 -> try - catch 문
    try {
        const data = fs.readFileSync('./231121_inde.html')  //의도적으로 에러냄(파일이름 오타)
        ...
        console.log(error)
        const data = fs.readFileSync('./231121_404.html')
        res.writeHead(200, { 'content-type': 'text/html; charset=utf8' });
        res.write(data);
        res.end();
    }
...
// conncection 이벤트: 클라이언트가 접속(클 - 서 연결됐을 때)했을 때 발생
server.on('connection', function (req, res) {
    console.log('connection 이벤트 발생!')
})

server.listen(PORT, function () {
    console.log(`serever listening on ${PORT}`)
})
